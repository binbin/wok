...
(function($) {
	...
	Kwick.prototype.calculatePanelSizes = function() {
		...
		if (opts.minSize === -1) {
			...
			// //最小是0
			// if(this.panelMinSize<=0){
			// 	this.panelMinSize=0
			// }
			// console.log(this.panelMinSize);
		...
		switch (this.opts.behavior) {
			case 'menu':
				this.$container.on('mouseleave', function() {
					...
					if(!$(this).data('i_kwicks')){
						if(self.opts.onExpand){
							self.opts.onExpand(self,$(this))
						}
						$(this).kwicks('expand');
					}
					
				}).click(function() {
					if(!self.opts.ignoreClick){
						$(this).kwicks('select');
					}	
				});
				break;
