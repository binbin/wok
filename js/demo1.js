$(function() {
	...
	(function(){
		(function(){
			...
			for(var i=1;i<7;i++){
				(function(i){
					var d=[]
					for(var j=1;j<6;j++){
						d.push('./photos/'+i+'/'+j+'.png')
					}
					d.push('./photos/totals/'+i+'.png')
					data.push(d)
				})(i);
			}
			var l=data.length
			var imgs=$('#pd_container img')
			$options.find('.shuffle').bind('click',function(e){
				...
				$.each(data[index],function(i,item){
					imgs.eq(i).attr('src',item)
				})
				if($options.find('.backdesk').is(':visible')){
					$options.find('.backdesk').click()
				}else{
					shuffle();
				}
			})
		...
		$options.find('.viewall').bind('click',function(e){
			stack(function(){
		...
		$options.find('.backdesk').bind('click',function(e){
			disperse(function(){
	...
	function stack(callback){
		...
		var windowsW 		= $container.width();
		var windowsH 		= $container.height(); 



		// $container.find('.pd_photo:last')
		$container.find('.pd_photo').each(function(i){
			var $photo 	= $(this);
	...
	function enlarge($photo){
		...
		$photo.show()//.css('rotate','0deg')
		if(ie)
		var param = {
			'width'	: '+=100px',
			'height': '+=100px',
			'top'	: parseInt((windowsH-100)/2 - (photoH+20)/2) + 'px', 
			'left'	: parseInt((windowsW-100)/2 - (photoW+20)/2) + 'px'
		};
		else
		var param = {
			'width'	: '+=100px',
		...
		$photo.transform({'rotate'	: 0 + 'deg'});
		$photo.animate(param,300,function(){
			idxLarge = $(this).index();
			// console.log("message");
		})
		$photo.find('img').animate({
			'width'	: '+=100px',
			'height': '+=100px'
		},300);
	}
	...
	function disperse(callback){
		...
		$container.find('.pd_photo').each(function(i){
			...
			if($photo.index() == idxLarge){
				...
				$photo.stop(true).animate(param,300, function(){
					shuffle();
					if(callback){
						callback()
					}
	...
	function shuffle(){
		...
		$container.find('.pd_photo').each(function(i){
            ...
            if(i+1 == $container.find('.pd_photo').length){
				$(this).hide();
				return;
			}
